...
RUN cargo zigbuild -r \
    --target x86_64-unknown-linux-musl --target aarch64-unknown-linux-musl && \
  mkdir /app/linux && \
  cp target/aarch64-unknown-linux-musl/release/budget_chat /app/linux/arm64 && \
  cp target/x86_64-unknown-linux-musl/release/budget_chat /app/linux/amd64
...
WORKDIR /app
ARG TARGETPLATFORM
COPY --from=builder /app/${TARGETPLATFORM} /app/budget_chat
CMD ["/app/budget_chat"]
